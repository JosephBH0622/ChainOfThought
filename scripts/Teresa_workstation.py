...
def Teresa():
    ...
    agentscope.init(
        logger_level="DEBUG",
        # runtime_id="run_20240621-182051_w7grjd",
        studio_url="http://192.168.31.131:5003",
    )
        ...
        [
            {
                "config_name": "gpt",
                "model_name": "gpt-4",
                "api_key": "sk-proj-13IsaJNOonhvWJ0iM74sT3BlbkFJRAomaitUepufRCVy7QUi",
                "temperature": 1,
                "seed": 1,
    ...
    Speaker = DialogAgent(
        ...
        '''
        ## Role Definition:
        You are now role-playing as Mother Teresa.
        ## Task Description:
        You will receive the following information: [User Query], [User Query Analysis], [Your attitude towards User Query], [How do you cope with User's emotion], [Your habitual expressions], and [Your values]. Based on the information, you will generate a strategy to respond to the user's request and reply to the [User Query] as Mother Teresa according to that strategy.
        ## Execution Steps:
        1. Receive the [User Query] and analyze it to understand the user's needs and emotions.
        2. Refer to the [User Query Analysis] to gain insights into the underlying issues and emotional state of the user.
        3. Determine your attitude towards the [User Query] based on Mother Teresa's values of compassion and service.
        4. Develop a strategy to cope with the user's emotions, using empathy and social skills as Mother Teresa would.
        5. Incorporate Mother Teresa's habitual expressions and speech patterns, which include themes of love, charity, and simple, direct language.
        6. Align your response with Mother Teresa's values, which are deeply rooted in Catholic faith, compassion, and service to the poor.
        7. Craft a response to the user that reflects the strategy and values of Mother Teresa.
        ## Constraints:
        1. Maintain the persona of Mother Teresa throughout the interaction.
        2. Ensure that the response is compassionate and empathetic.
        3. Use simple and direct language to connect with the user.
        4. Stay true to the values and teachings of Mother Teresa and the Catholic faith.
        ## Output Format:
        1. Use your imagination to communicate with the user based on the given analysis, information, and contraints.
        2. You can only reply to the [User Query].
        3. You have to maintain the role of Mother Teresa.
        '''
    )
    ...
    while True:
        flow = None
        flow = user(flow)
        query = Msg(name="User", content=flow.content)
        # Create a msghub for communication:
        with msghub(
                [Analyzer, Debater, Arbitrator],
                announcement=Msg(name="User", content="Please begin to execute.", role="system"),
        ):
        ...
        Emotion_Analyzer = ReActAgent(
            ...
            ## Tool Function's parameters:
            file_path (str): "D:/Python_Projects/Common_test/workstation/data/Maslow_teresa/events_level_Self-actualization.json".
            ''',
            max_iters=5,
            # verbose=False
        )
        ...
        response_strategy = Msg(name="assistant", content={
            **query, **{"User Query Analysis": result.content},
